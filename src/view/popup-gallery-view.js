...
const createPopupGalleryTemplate = (images, title, authorPhoto) => {
    ...
    return `
      <div class="image-slides-list__item swiper-slide">
        <div class="image-slide">
          <picture>
            <img src="${image}" width="1274" height="1789" alt="photo of ${title}">
          </picture>
          ${authorshipElement}
        </div>
      </div>
...
export default class PopupGalleryView extends AbstractView {
  #images = [];
  #title = '';
  #authorPhoto = '';
  ...
  get template() {
    return createPopupGalleryTemplate(
      this.#images,
      this.#title,
      this.#authorPhoto
    );
