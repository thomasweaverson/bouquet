  ...
  <li class="popup-deferred__item">
    <div class="deferred-card ${isEditing ? 'is-loading' : ''}">
      <div class="deferred-card__img">
        <picture>
          <img src="${previewImage}" width="344" height="408" alt="букет ${title}">
        </picture>
      </div>
      ...
      <div class="deferred-card__count">
        <button class="btn-calculate btn-calculate--decrease" type="button" ${isEditing ? 'disabled' : ''}>
          <svg width="30" height="27" aria-hidden="true">
            <use xlink:href="#icon-minus"></use>
          </svg>
        </button>
        <span>${count}</span>
        <button class="btn-calculate btn-calculate--increase" type="button" ${isEditing ? 'disabled' : ''}>
      ...
      <div class="deferred-card__price">
        <b class="price price--size-middle-p">${formatPrice(price)}<span>Р</span></b>
      </div>
